//! PAMHC model-property header collection editor — load `.pamhc`
//! files from the game's PAZ archives, edit the structured five-section
//! container, and deploy the result as a PAZ overlay.
//!
//! Format definition: the `pamhc` section of
//! `tools/mod-workbench/PALEVEL_PAMHC_PAB_FORMAT_RESEARCH.md`.
//! Parser implementation:
//! [`dmm_parser_rust_only::tables::pamhc::PamhcFile`]. The struct
//! round-trips byte-for-byte against vanilla, so editing one field
//! doesn't disturb the surrounding bytes.
//!
//! ## PAZ enumeration
//!
//! Mirrors `pappt_editor::enumerate_pappt_files`: walks every numeric
//! 4-digit overlay group under the configured Game Directory, parses
//! its PAMT, and collects every `.pamhc` file. In retail there is
//! exactly one — `miscellaneous/modelpropertyheadercollection.pamhc` —
//! but the picker is a list rather than a hard-coded path so the
//! panel handles future variants without code changes.
//!
//! ## Deploy
//!
//! Same overlay flow as the pappt / paatt editors: write a fresh PAZ
//! overlay group at `<game_dir>/<overlay_group>/` containing the
//! modified `.pamhc` at the same internal path the vanilla file lives
//! at, then front-insert the group into PAPGT so it wins lookup. The
//! default overlay group is `"0072"` — one above pappt's `"0071"`,
//! kept distinct from the other tool groups so multiple workbench
...
/// One `.pamhc` file located inside a PAZ archive. Returned by
/// [`enumerate_pamhc_files`]; the editor stores these in its picker
/// so the user can choose by display name without remembering the
/// internal PAZ directory.
#[derive(Clone, Debug)]
pub struct PamhcPazEntry {
    /// PAZ group folder name (e.g. `"0008"`).
    pub group: String,
    /// Internal PAMT directory path (e.g. `"miscellaneous"` for the
    /// retail `modelpropertyheadercollection.pamhc`).
    pub dir_path: String,
    /// File name including the `.pamhc` extension.
    pub filename: String,
}
...
/// PAMT, and collect every `.pamhc` file. Sorted by group then
/// directory then filename so the resulting list is stable across
/// runs.
///
/// Errors loading individual PAMTs are non-fatal — we skip the
/// broken group and continue, returning what we found from the rest.
/// Matches the "best-effort, never block on a missing overlay"
/// behaviour of [`crate::pappt_editor::enumerate_pappt_files`].
pub fn enumerate_pamhc_files(game_dir: &Path) -> io::Result<Vec<PamhcPazEntry>> {
    let mut found: Vec<PamhcPazEntry> = Vec::new();
    ...
    for entry in entries.flatten() {
        ...
        for dir in &pamt.directories {
            for file in &dir.files {
                if file.name.to_ascii_lowercase().ends_with(".pamhc") {
                    found.push(PamhcPazEntry {
                        group: name.clone(),
...
/// Read the raw bytes for a single `.pamhc` file from the game's PAZ.
/// Used by the panel to populate its session on file selection.
pub fn read_pamhc_from_paz(game_dir: &Path, entry: &PamhcPazEntry) -> io::Result<Vec<u8>> {
...
/// Deploy a single modified `.pamhc` as a PAZ overlay.
///
/// Builds `<game_dir>/<overlay_group>/0.paz + 0.pamt` containing just
/// `<dir_path>/<filename>` with the new bytes, then front-inserts the
/// group into PAPGT so the next game launch loads our pamhc instead
/// of the vanilla one.
///
/// `overlay_group` should be a 4-digit numeric folder name not
/// currently used by the game (default `"0072"` for the pamhc editor —
/// one above pappt's `"0071"`, kept distinct from other tools' groups
/// so multiple workbench overlays can coexist).
pub fn deploy_pamhc_overlay(
    game_dir: &Path,
    entry: &PamhcPazEntry,
    modified_bytes: &[u8],
    overlay_group: &str,
    ...
    // `pappt_editor::deploy_pappt_overlay`. The encrypt material is
    // shared across the install in practice so the source group
    // choice doesn't matter for correctness.
...
/// removing its PAPGT entry. Mirrors `pappt_editor::restore_pappt_overlay`
/// but scoped to the pamhc editor's overlay group.
pub fn restore_pamhc_overlay(game_dir: &Path, overlay_group: &str) -> io::Result<()> {
    let group_dir = game_dir.join(overlay_group);
    if group_dir.exists() {
        fs::remove_dir_all(&group_dir)?;
    }
...
/// checksum. Same shape as the pappt deploy path so the workbench
...
/// exist. Mirrors the path used by `pappt_editor::restore_pappt_overlay`.
