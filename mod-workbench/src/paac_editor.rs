//! PAAC action-chart editor — load `.paac` files from the game's PAZ
//! archives, edit floats / inline transitions, and deploy the result as
//! a PAZ overlay.
//!
//! On-disk format is parsed by
//! [`dmm_parser_rust_only::tables::paac::PaacFile`]. The parser keeps
//! the original bytes verbatim in [`PaacFile::raw`], so byte-level
//! mutations (`patch_float`, `patch_transition`) survive a re-parse and
//! the file round-trips when no edits are made.
//!
//! Distinct from the per-weapon attack-info `.paatt` editor — `.paac`
//! files describe the action-chart state machine, while `.paatt` files
//! hold projectile attribute tables. Both ship as PAZ overlays with the
//! same shape as the XML editor's overlay path.
//!
//! ## PAZ enumeration
//!
...
//! its PAMT, and collects every `.paac` file. Sorted by group +
//! directory + name so the resulting list is stable across runs.
//!
//! ## Deploy
//!
//! Same overlay flow as paatt: write a fresh PAZ overlay group at
//! `<game_dir>/<overlay_group>/` containing the modified `.paac` at the
//! same internal path the vanilla file lives at, front-insert the
//! group into PAPGT so it wins lookup. Default overlay group is
//! `"0067"` — one above paatt's `0066` so multiple workbench overlays
//! can coexist.
...
/// One `.paac` file located inside a PAZ archive. Returned by
/// [`enumerate_paac_files`]; the editor stores these in its picker so
/// the user can choose by display name without remembering the
/// internal PAZ directory.
#[derive(Clone, Debug)]
pub struct PaacPazEntry {
    /// PAZ group folder name (e.g. `"0010"`).
    pub group: String,
    /// Internal PAMT directory path (e.g. `"actionchart"`).
    pub dir_path: String,
    /// File name including the `.paac` extension.
    pub filename: String,
}
...
/// PAMT, and collect every `.paac` file. Sorted by group then filename
/// so the resulting list is stable across runs.
///
/// Errors loading individual PAMTs are non-fatal — we skip the broken
/// group and continue, matching the "best-effort" behaviour of
/// [`crate::paatt_editor::enumerate_paatt_files`].
pub fn enumerate_paac_files(game_dir: &Path) -> io::Result<Vec<PaacPazEntry>> {
    let mut found: Vec<PaacPazEntry> = Vec::new();
    ...
    for entry in entries.flatten() {
        ...
        for dir in &pamt.directories {
            for file in &dir.files {
                if file.name.to_ascii_lowercase().ends_with(".paac") {
                    found.push(PaacPazEntry {
                        group: name.clone(),
...
/// Read the raw bytes for a single `.paac` file from the game's PAZ.
/// Used by the editor to populate its session on file selection.
pub fn read_paac_from_paz(game_dir: &Path, entry: &PaacPazEntry) -> io::Result<Vec<u8>> {
...
pub fn deploy_paac_overlay(
    ...
    // `paatt_editor::deploy_paatt_overlay`.
    ...
    builder.add_file(&entry.dir_path, &entry.filename, paac_bytes)?;
...
/// removing its PAPGT entry. Mirrors
/// [`crate::paatt_editor::restore_paatt_overlay`] but scoped to the
/// paac editor's overlay group.
pub fn restore_paac_overlay(game_dir: &Path, overlay_group: &str) -> io::Result<()> {
    let group_dir = game_dir.join(overlay_group);
    if group_dir.exists() {
        fs::remove_dir_all(&group_dir)?;
    }
...
/// exist. Mirrors the path used by
/// [`crate::paatt_editor::restore_paatt_overlay`].
